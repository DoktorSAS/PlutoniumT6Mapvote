...
local HUD_OpenIngameMenu, HUD_SetupEventHandlers, HUD_KillcamUpdate, HUD_UpdateKillstreakHud, HUD_ForceKillKillstreakHud, HUD_SelectingLocationUpdate, HUD_GameEndedUpdate, HUD_ReloadShoutcasterHud, HUD_UpdateVehicleHud, HUD_FactionPopup, FactionPopupAddTextEvent, f0_local11, f0_local12 =
	nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil
function HUD_IngameMenuClosed() end

local DOK_OpenMapvote = function(HUDWidget, ClientInstance)
	HUDWidget:processEvent(ClientInstance)
	HUDWidget:openPopup("MapVote", ClientInstance.controller)
end
...
LUI.createMenu.HUD = function(LocalClientIndex)
	...
	HUDWidget:registerEventHandler("open_custom_in_game_menu", function(HUDWidget, ClientInstance)
		--print("open_custom_in_game_menu")
		if ClientInstance.data ~= nil then
			--print("if ClientInstance.data ~= nil then")
			--print(Engine.GetIString(ClientInstance.data[1], "CS_LOCALIZED_STRINGS"))
			HUDWidget:openPopup(Engine.GetIString(ClientInstance.data[1], "CS_LOCALIZED_STRINGS"), ClientInstance.controller)
		end
	end)
...
function HUD_FirstSnapshot_Common(HUDWidget, ClientInstance)
	...
	if CoD.isZombie == true then
		...
		if
			f17_local2 == 1
			and (CoD.isOnlineGame() == false or Engine.GameModeIsMode(CoD.GAMEMODE_PRIVATE_MATCH) == false)
		then
...
function HUD_OpenScoreBoard(HUDWidget, ClientInstance)
	if
		CoD.isZombie == true
		and CoD.InGameMenu.m_unpauseDisabled ~= nil
		and CoD.InGameMenu.m_unpauseDisabled[ClientInstance.controller + 1] ~= nil
		and CoD.InGameMenu.m_unpauseDisabled[ClientInstance.controller + 1] > 0
	then
		return
...
HUD_KillcamUpdate = function(HUDWidget, ClientInstance)
	if UIExpression.IsVisibilityBitSet(ClientInstance.controller, CoD.BIT_IN_KILLCAM) == 1 then
		if HUDWidget.killcamHUD then
			if
				HUDWidget.killcamHUD.isFinalKillcam
				~= UIExpression.IsVisibilityBitSet(ClientInstance.controller, CoD.BIT_FINAL_KILLCAM)
			then
				HUD_StopKillcamHud(HUDWidget, ClientInstance)
			elseif
				HUDWidget.killcamHUD.isRoundEndKillcam
				~= UIExpression.IsVisibilityBitSet(ClientInstance.controller, CoD.BIT_ROUND_END_KILLCAM)
			then
				HUD_StopKillcamHud(HUDWidget, ClientInstance)
			end
		end
...
local f0_local18 = function(f32_arg0)
	local f32_local0
	if
		Engine.IsSplitscreen() ~= false
		or Engine.IsDemoShoutcaster() ~= true
			and (UIExpression.IsVisibilityBitSet(f32_arg0, CoD.BIT_SPECTATING_CLIENT) ~= 1 or UIExpression.IsVisibilityBitSet(
				f32_arg0,
				CoD.BIT_TEAM_SPECTATOR
			) ~= 1 or UIExpression.IsVisibilityBitSet(f32_arg0, CoD.BIT_GAME_ENDED) ~= 0 or UIExpression.IsVisibilityBitSet(
				f32_arg0,
				CoD.BIT_UI_ACTIVE
			) ~= 0)
	then
...
local f0_local19 = function(f35_arg0)
	local f35_local0
	if
		UIExpression.IsVisibilityBitSet(f35_arg0, CoD.BIT_SELECTING_LOCATION) ~= 1
		or UIExpression.IsVisibilityBitSet(f35_arg0, CoD.BIT_SPECTATING_CLIENT) ~= 0
		or UIExpression.IsVisibilityBitSet(f35_arg0, CoD.BIT_IS_DEMO_PLAYING) ~= 0
		or UIExpression.IsVisibilityBitSet(f35_arg0, CoD.BIT_SCOREBOARD_OPEN) ~= 0
	then
		f35_local0 = false
	else
		f35_local0 = true
...
HUD_FactionPopup = function(HUDWidget, ClientInstance)
	...
	if f40_local0 == "TEAM_ALLIES" then
		...
		f40_local5 = UIExpression.ToUpper(
			nil,
			Engine.Localize(
				Engine.GetIString(ClientInstance.data[1], "CS_LOCALIZED_STRINGS"),
				Dvar.g_TeamName_Allies:get()
			)
		)
...
function HUD_IsFFA()
	...
	if CoD.isZombie == true then
		...
		if
			GamemodeGroup ~= CoD.Zombie.GAMETYPEGROUP_ZCLASSIC
			and GamemodeGroup ~= CoD.Zombie.GAMETYPEGROUP_ZSURVIVAL
		then
			IsFFa = true
		end
	elseif
		UIExpression.DvarString(nil, "ui_gametype") == "dm"
		or UIExpression.DvarString(nil, "ui_gametype") == "hcdm"
		or UIExpression.DvarString(nil, "ui_gametype") == "hack"
	then
		IsFFa = true
	end
	return IsFFa
...
function HUD_Handle_ChooseClass_HotKey(HUDWidget, ClientInstance)
	if
		UIExpression.Team(ClientInstance.controller, "name") ~= "TEAM_SPECTATOR"
		and CoD.IsWagerMode() == false
		and not (Engine.GetGametypeSetting("disableClassSelection") == 1)
	then
